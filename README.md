...
Content Publisher 是一个面向长期纯个人使用的技术内容生产与多平台发布工具。系统可以从 Git 仓库、结构化主题或公开网站提取受控事实，也支持直接编写或粘贴自定义文章/笔记；前述来源可通过 OpenAI Chat Completions 兼容服务生成中英文内容，经过草稿自动保存、人工编辑和版本确认后，以官方 API 或合规人工流程发布到不同平台。

Portal 主流程已经迁移为纯个人内容确认，不再提供待审核、批准或驳回入口。日常路径是写稿、保存并准备发布、按常用渠道覆盖。当前代码仍保留多租户、Viewer/Editor/Admin 角色，以及 REST `approve/reject`、`APPROVED/REJECTED` 状态和部分兼容错误码；这些只用于历史合同与旧数据兼容，不代表未来产品方向。Portal 账户菜单不再展示租户和角色。
...
| 能力域 | 状态 | 说明 |
|---|---|---|
| 四类内容来源 | 已实现 | Git 安全导入、结构化主题、带 SSRF 防护的公开网站抓取、自定义文章/笔记直接编写与粘贴 |
| AI 与 SEO 辅助 | 已实现 | 租户级 OpenAI 兼容配置、中英文结构化输出、确定性质量校验 |
| 草稿与预设 | 已实现 | 服务端用户级草稿、浏览器自动保存、版本基线冲突检查、生成预设复用 |
| 内容与版本 | 已实现 | 不可变版本、保存并确认、已发布显式修订、分栏预览、配图、中英一键生成；REST 审核/驳回仅作历史兼容 |
| 持久化任务 | 已实现 | 幂等、配额、进度、批次、定时、租约、退避、取消、恢复和通用失败重放 |
| 发布计划 | 已实现 | 浏览器时区输入转 UTC、DST 间隙/歧义校验、发布日历 |
| API 渠道 | 已实现 | 9 个可新接入渠道，Medium 只保留合法存量账号；支持连接管理、安全移除与历史记录保留，无法申请 API 时可切换人工发布 |
| 人工渠道 | 已实现 | 24 个具有官方创作入口的平台：17 个纯人工渠道加 7 个同时支持 API 的渠道，可配置启停与默认设置，并通过独立持久浏览器 Profile 完成发布 |
...
- Admin 可以移除不再使用或无法继续授权的 API 账号；系统采用软删除，立即清除加密凭据并从账号列表、发布选择、巡检和监控中隐藏，但保留历史发布记录及原账号显示名称。
- 外部结果不确定时不自动盲目重试；确认第三方未成功后再使用任务重放。

### 人工发布

- 在 `/channels?view=manual` 配置 24 个具有官方创作入口的平台；未保存配置的平台默认启用。DEV、WordPress、GitHub Discussions、Twitter/X、Reddit、Hashnode 和 Medium 即使没有 API 账号也可直接人工发布。
- Discourse、Mastodon 和 Ghost 需要用户自己的站点或实例地址，当前没有可安全复用的固定编辑入口，因此不进入人工发布列表。
...
- 人工工作区主路径是“复制并打开编辑器”，再回填公开 URL；五项进度仍会保存。
...
- Portal 和发布门禁已采用个人确认语义，账户菜单不再展示租户/角色；多租户、Viewer/Editor/Admin 仍是历史兼容结构，尚未收敛为单主体数据模型。
- REST `approve/reject` 已标记 deprecated，`APPROVED/REJECTED` 和错误码 `ARTICLE_NOT_APPROVED` 仍保留兼容；其中错误码当前表示“内容尚未确认”，不能按字面理解为仍需要审批。
- Flyway V22 会把迁移时已有的 `APPROVED` 文章回填为 `READY`；V23 为 API 渠道账号增加软删除字段。旧版本应用回滚必须先评估新状态和新 Schema 的兼容性。
- 主加密密钥没有版本化和在线迁移。
