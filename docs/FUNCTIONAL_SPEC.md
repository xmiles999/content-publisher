...
共享导航按工作空间、内容生产、发布管理、运行维护和系统设置分组。当前页面使用语义化当前项标识，所在分组自动展开，并在短视口中滚动到可见区域；用户手动切换的折叠状态在浏览器本地持久化。移动端导航切换为可关闭抽屉，支持 Escape、焦点约束和关闭后的焦点恢复。
...
- 自动化设置页同时展示项目、主题和网站三类来源的内置或自定义生成预设，并提供通知 Webhook 管理；页面渲染失败不得影响 Admin 进入系统设置。
- 动作台汇总待审核文章、失败任务、已审核待发布文章和开放通知。
- 通知使用租户内 `dedupKey` 去重，可被当前用户确认；巡检恢复会关闭对应失败通知。
- 渠道巡检只处理超过陈旧阈值的账号，并在状态首次失败或从失败恢复时通知。
- Admin 可配置租户 Webhook；每个“通知 × 端点”只有一个投递记录。
...
当前 OpenAPI 快照包含 45 个 REST 操作，覆盖项目、文章与草稿、生成预设、渠道账号、发布、任务与重放、通知/Webhook、人工发布进度、Markdown 和监控。
