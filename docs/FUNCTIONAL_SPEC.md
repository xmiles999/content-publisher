...
| 动作台 | 聚合待审核、失败任务、待人工发布和站内通知的运营入口 |
...
| Admin | 审核、渠道、AI、生成预设和 Webhook 设置、软删除与恢复，以及所有 Editor 能力 |
...
3. 用户可以在工作区继续调整派生内容。
4. 用户复制内容并打开官方创作入口。
5. 工作区按当前主体保存“复制标题、复制正文、打开编辑器、检查格式、已发布”五项进度。
6. 用户在第三方平台登录、排版和发布。
7. 回填 HTTPS 公开 URL。
8. 服务端校验 URL 主机与渠道允许域名。
9. 保存最终派生内容快照、操作者和发布时间。
...
导航计数按当前租户实时聚合：动作台为待审核/驳回文章、失败任务、运行超时任务、未完成人工发布、异常渠道账号和开放通知之和；待发布为 `APPROVED` 文章；任务队列为 `PENDING`、`RUNNING`、`RETRY_WAIT`、`FAILED` 任务。
...
1. 独立审核历史和审核意见查询。
2. 模型调用成本和更细粒度的 Prompt 生命周期治理。
3. 可配置工作器并发和独立死信表。
4. 主密钥版本化迁移和 OAuth 到期提醒。
5. UTM、效果数据回收和分析。
6. 浏览器级端到端测试和持续的真实 PostgreSQL 锁语义验证。
7. Webhook 预解析 IP 绑定；当前仍需出站 ACL 缩小 DNS rebinding 时间窗。
