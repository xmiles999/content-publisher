...
| 人工平台配置 | 个人使用者为纯人工渠道保存启停、账号别名、默认标签/栏目、备注、排序和登录确认时间 |
...
- “确认当前浏览器已登录”只更新时间标记；它不读取、探测或保证第三方登录会话。
- 官方入口只能来自 `ChannelCatalog`，用户不能保存任意平台 URL。
- 系统不保存第三方用户名密码、Cookie、Session、验证码或恢复码；登录状态由浏览器和第三方平台维护。
...
- 页面和表单不包含密码、Cookie、Session、验证码或恢复码字段；登录确认只保存时间，不宣称自动检测会话。
